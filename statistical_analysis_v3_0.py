...
def parse_commorbidities(samples,commorbidities_filename,commorbidities_types):
	...
	with open(commorbidities_filename) as commorbidities_fname:
		for line in csv.reader(commorbidities_fname, delimiter="\t"):
			...
			for i in range(len(line)):
				if commorbidities_types[i]=="1":
					commorbidities[i].append(int(line[i].strip()))
				else:
...
def filter_proteomics_dataset(dataset_initial,proteins,labels,unique_labels,percentage,output_message):
	new_data=list()
	selected=0
	new_proteins=list()
	missing_proteins=0
	...
	for i in range(len(dataset_initial)):
		...
		for j in range (len(dataset_initial[0])):
			...
			if labels[j]==unique_labels[0]:
				...
				total2+=1
		if (((missing/float(len(dataset_initial[0])))<(percentage)) or ((missing1/float(total1)<=0.1) and (missing2/float(total2)>0.9)) or ((missing2/float(total2)<=0.1) and (missing1/float(total1)>0.9))):
			#print(i)
			new_data.append([])
			for k in range(len(dataset_initial[i])):
				new_data[selected].append(dataset_initial[i][k])
...
def print_volcano_plots(proteins, pvals, log_fold_changes, filename,filename2,p_value_threshold):
	...
	limit1=-1*max(absolute_log_fold_changes)
	limit2=max(absolute_log_fold_changes)
    ...
    #                                      colormap_labels]) 
	gp=ggplot2.ggplot(data=df) +ggplot2.aes_string(x='log2FC', y='MinusLog10Pvals', label='Ids', colour='colors') +ggplot2.xlim(limit1, limit2) +ggplot2.geom_point() +ggplot2.scale_colour_manual("", 
                                  values=colormap,
                                  breaks=colormap.names,
                                  labels=[elt[1] for elt in 
                                          colormap_labels]) 
	...
	ro.r('p2<-expression(paste(log[2], \"(FC)\" ))')
	r.png(filename)
	gp=ggplot2.ggplot(data=df) +ggplot2.aes_string(x='log2FC', y='MinusLog10Pvals', label='Ids', colour='colors') + ggplot2.xlim(limit1, limit2) +ggplot2.geom_point() +ggplot2.geom_text(colour ='black', check_overlap='True') +ggplot2.scale_colour_manual("", 
                                  values=colormap,
                                  breaks=colormap.names,
                                  labels=[elt[1] for elt in 
                                          colormap_labels]) 
...
def print_boxplots(new_proteins,proteins, data, values_list, labels_1, folder, pvals,p_value_threshold):
	...
	for j in range(len(proteins)):
		...
		if pvals[j]<p_value_threshold:
			...
			mpl_fig = plt.figure(figsize=(4.2,5))
			mpl_fig.subplots_adjust(bottom=0.25,left=0.2)
			ax = mpl_fig.add_subplot(111)
			bp=ax.boxplot(splitted_data, widths=0.2,notch=True,patch_artist=True, positions= ((np.arange(2))+0.6)/(2.6) )
			#ax.set_aspect(1.2)
			...
			ax.set_title(proteins[j],fontsize=15)
			ax.set_ylabel('Logged Relative Quantities',fontsize=15)
			ax.set_xlabel(values_list[0]+' vs '+values_list[1]+'\nAdgusted Pvalue:'+str(round(pvals[j],6)),fontsize=15)
			labels = [item.get_text() for item in ax.get_xticklabels()]
			labels[0] = values_list[0]+'\n(n=' +str(num_of_cat[0])+')'
			labels[1] = values_list[1]+'\n(n=' +str(num_of_cat[1])+')'

			ax.set_xticklabels(labels,fontsize=15)
			ax.set_xticks(((np.arange(2))+0.6)/(2.6))
...
def perform_missing_values_imputation(dataset_initial,missing_imputation_method, labels, unique_labels, folder_name,output_message):
	...
	for i in range(len(dataset_initial)):
		...
		if ((missing1/float(total1)<=0.1) and (missing2/float(total2)>0.9)):
			
			for j in range (len(dataset_initial[0])):
				if (dataset_initial[i][j]=='') or dataset_initial[i][j]==-1000:
					
					if (labels[j]==unique_labels[1]):
						dataset_initial[i][j]=0
		elif ((missing2/float(total2)<=0.1) and (missing1/float(total1)>0.9)):
			for j in range (len(dataset_initial[0])):
				if (dataset_initial[i][j]=='') or dataset_initial[i][j]==-1000:
...
def normalize_dataset(dataset_initial, folder_name,output_message,normalization_method):	
	if normalization_method=='1':
		...
		maximums=[-1000.0]*(len(dataset_initial))
		minimums=[1000.0]*(len(dataset_initial))
		for i in range(len(dataset_initial)):
			for j in range((len(dataset_initial[0]))):
				if(dataset_initial[i][j]!="" or dataset_initial[i][j]!=-1000):
		...
		for i in range(len(dataset_initial)):
			for j in range((len(dataset_initial[0]))):
				if (dataset_initial[i][j]!="" or dataset_initial[i][j]!=-1000):
					dataset_initial[i][j]=0+(1/(maximums[i]-minimums[i]))*(float(dataset_initial[i][j])-minimums[i])
...
def spearman_correlation(markers,data,filesuffix):
	...
	spearman_pvalues= open(filesuffix+'spearman_table_pvalues.txt','w')
	spearman_network=open(filesuffix+'spearman_network.txt','w')
	num_of_pairs=0
	for i in range(len(markers)):
		spearman_table.write('\t'+markers[i])
		spearman_pvalues.write('\t'+markers[i])
	spearman_table.write('\n')
	spearman_pvalues.write('\n')
	for i in range(len(markers)):
		spearman_table.write(markers[i])
		spearman_pvalues.write(markers[i])
		for j in range(len(markers)):
			if j>i:
				...
				spearman_pvalues.write('\t'+str(p))
				if p<0.05 and r>0.5:
					spearman_network.write(markers[i]+'\t'+markers[j]+'\t'+str(r)+'\n')
			elif j==i:
				spearman_table.write('\t1')
				spearman_pvalues.write('\t0')
			else:
				spearman_table.write('\t')
				spearman_pvalues.write('\t')
		spearman_table.write('\n')
		spearman_pvalues.write('\n')
	spearman_table.close()
	spearman_network.close()
	spearman_pvalues.close()
if __name__ == "__main__":
	...
	#unique_labels=['1','0']
	#unique_labels=['before_bariatric','18_months_after']
	print(unique_labels)
	
	if commorbidities_flag!=0:
		[commorbidities,commorbidities_flag]=parse_commorbidities(samples,commorbidities_filename,commorbidities_types)
	else:
		commorbidities=list()
	output_message=''
	if commorbidities_flag==0:
	...
	[data_filtered,markers,output_message] = filter_proteomics_dataset(data,markers,labels,unique_labels,missing_threshold,output_message)
	
	dataset_imputed=data_filtered
	if missing_imputation_method!="0":
		[dataset_imputed,output_message]=perform_missing_values_imputation(data_filtered,missing_imputation_method,labels,unique_labels, folder_name,output_message) 
	...
	print_data(dataset_imputed,markers,samples,folder_name, 'preprocessed_data2.txt')
	...
	if test_flag==1:
		...
		# if labels[0]==unique_labels[1]:
			# for k in range(len(diff_table[0])):
				# diff_table[0][k]=diff_table[0][k]*(-1)
		...
		print_beanplots(markers,diff_proteins, dataset_imputed, unique_labels, labels, folder_name+unique_labels[0]+'VS'+unique_labels[1]+'_boxplots_corrected/',diff_table[4],p_value_threshold)
		print_beanplots(markers,diff_proteins, dataset_imputed, unique_labels, labels, folder_name+unique_labels[0]+'VS'+unique_labels[1]+'_boxplots/',diff_table[3],p_value_threshold)
		filtered_data=list()
		position=0
		...
		print_beanplots(markers,markers, dataset_imputed, unique_labels, labels, folder_name+unique_labels[0]+'VS'+unique_labels[1]+'_boxplots_corrected/',pvals2,p_value_threshold)
		print_beanplots(markers,markers, dataset_imputed, unique_labels, labels, folder_name+unique_labels[0]+'VS'+unique_labels[1]+'_boxplots/',pvals,p_value_threshold)
		filtered_data=list()
		position=0
		diff_proteins_corrected=list()
		for i in range(len(markers)):
	...
	out_file.close()
	spearman_correlation(markers,dataset_imputed,folder_name+'network')
